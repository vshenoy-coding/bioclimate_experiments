...
if __name__ == "__main__":
    # Create the area for the video/logs once.
    results_area = widgets.Output()

    while True:
        # Clear previous video before starting new prompt.
        # The previous prompt location text stays as a record.
        results_area.clear_output()
        
        # Get input first. 
        # Since this is outside any 'with' block, it stays at the top.
        ...
        # 3. Now show the results area below the input.
        display(results_area)
        
        with results_area:
            # Now, "Initializing..." will strictly appear here, below your input.
            out_video = generate_deep_time_path(lat, lon, name)

            if out_video and os.path.exists(out_video):
                display(Video(out_video, embed=True, width=600))
          
            # This pause happens at the very bottom of the results area.
            input("\nPress Enter when you are ready to search for another location...")

        # Clear everything now, instead of waiting for the next lat input.
        # clear_output(wait=True)

#########################################################################################
# Previous code where input box was continuously hidden by video.
# To fix this issue, used widget from ipywidgets to define container so that it did
# not move around and hide the input box.
#########################################################################################
